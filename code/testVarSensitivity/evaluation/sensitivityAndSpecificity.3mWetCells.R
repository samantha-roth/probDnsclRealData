...
for(v in 1: length(var_samples)){
  ...
  #specificity
  spec_LRW<- length(correctNoFloodInds)/length(noFlood5mInds) #0.9881306
  ...
  save(spec_LRW,sens_LRW,nFlood,nNoFlood,flood5mInds,noFlood5mInds,
       ...
       file=paste0("data/var_sample",v,"/wetCellAccuracy_.3mflood_10mto5m.RData"))
  
  pFloodWet<- 1-pNoFlood
  save(pFloodWet,file=paste0("data/var_sample",v,"/p.3mflood_10mto5m.RData"))
