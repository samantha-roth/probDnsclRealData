...
for(f in 1:length(flood)){
  ...
  for(v in 1: length(var_samples)){
    ...
    save(time_estProbFloodIndsofInterest, 
         file= paste0("data/",flood[f],"/var_sample",v,"/time_estProbFloodIndsofInterest_10mto5mElev_QGIS.RData"))
    ...
    save(indsProbNotFloodgeq.5,indsProbFloodgeq.5,
         ...
         file=paste0("data/",flood[f],"/var_sample",v,"/estProbFloodIndsofInterest_10mto5mElev_QGIS.RData"))
    
    ################################################################################
    pFloodDry<- 1-totProbleq0
    save(pFloodDry,file=paste0("data/",flood[f],"/var_sample",v,"/pFloodDry_10mto5mElev_QGIS.RData"))
