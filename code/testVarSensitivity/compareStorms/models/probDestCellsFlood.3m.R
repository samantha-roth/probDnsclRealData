...
for(f in 1:length(flood)){
  ...
  for(v in 1: length(var_samples)){
    ...
    save(time_estProbFloodIndsofInterest, 
         file= paste0("data/",flood[f],"/var_sample",v,"/time_estProbFlood.3mIndsofInterest_10mto5mElev_QGIS.RData"))
    ...
    save(indsProbNotFloodgeq.5,indsProbFloodgeq.5,
         ...
         file=paste0("data/",flood[f],"/var_sample",v,"/estProb.3mFloodIndsofInterest_10mto5mElev_QGIS.RData"))
    
    ################################################################################
    pFloodDry<- 1-totProbleq.3
    save(pFloodDry,file=paste0("data/",flood[f],"/var_sample",v,"/p.3mFloodDry_10mto5mElev_QGIS.RData"))
