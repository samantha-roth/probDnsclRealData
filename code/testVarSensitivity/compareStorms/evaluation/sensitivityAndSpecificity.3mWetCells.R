...
for(f in 1:length(flood)){
  ...
  for(v in 1: length(var_samples)){
    ...
    save(spec_LRW,sens_LRW,nFlood,nNoFlood,flood5mInds,noFlood5mInds,
         ...
         file=paste0("data/",flood[f],"/var_sample",v,"/wetCellAccuracy_.3mflood_10mto5m.RData"))
    
    pFloodWet<- 1-pNoFlood
    save(pFloodWet,file=paste0("data/",flood[f],"/var_sample",v,"/p.3mFloodWet_10mto5m.RData"))
