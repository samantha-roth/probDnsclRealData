...
for(f in 1:length(flow)){
  ...
  length(which(good5minds%in%good5minds[c(destIndsinGoodInds30m,floodInds30mat5m)]))
  
  ################################################################################
  #get flooded and not flooded locations within the business region of interest
  floodInds<- good5minds[floodInds30mat5m]
  ...
  load(paste0("probabilisticDownscaling/data/",flow[f],"/simObs/estProbFloodIndsofInterest_30mto5m.RData"))
  ...
  correctFloodDryInds<- destInds30m[indsFloodCorrect]
  correctNoFloodDryInds<- destInds30m[indsNotFloodCorrect]
  incorrectNoFloodDryInds<- destInds30m[setdiff(indsFlood,indsFloodCorrect)]
  incorrectFloodDryInds<- destInds30m[setdiff(indsNotFlood,indsNotFloodCorrect)]
  
  label<- rep(NA, nrow(coords.5m))
  
  #label[floodInds30mat5m]<- "wet"
  #label[-floodInds30mat5m]<- "dry"
  ...
  filename<- paste0("probabilisticDownscaling/plots/",flow[f],"/BusAreaAccSpatSts_30mto5m.jpeg")
