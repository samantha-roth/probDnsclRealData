...
for(f in 1:length(flow)){
  ...
  #now that we've established the 5m points where 10m and 10m cost values are defined
  ...
  floodBin10mat5mVals<- extract(floodBin10mat5m,coords5m[goodInds5m,])
  floodBin5mVals<- extract(floodBin5m,coords5m[goodInds5m,])
  
  floodBin10mat5mVals<- c(as.matrix(floodBin10mat5mVals))
