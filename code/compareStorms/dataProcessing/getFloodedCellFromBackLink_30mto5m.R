...
#get the 5m source cell within the 30m flooded area that maps to each 5m destination cell
...
for(f in 1:length(flow)){
  ...
  backlink30mat5m<- rast(paste0("Documents/ArcGIS/Projects/Storm",flow[f],"/BackLink_30mat5m.tif"))
  backLinkVals30mat5m<- c(as.matrix(extract(backlink30mat5m,coords.5m)))
  ...
  ordsDestLocs30m<- ords.5m[destInds30m,]
  ...
  source_value <- 0  
  sourcesForDests30m<- matrix(NA, nrow= length(destInds30m), ncol= 2)
  
  start<- proc.time()
  for(i in 1:length(destInds30m)){
    #for(i in 1:100){
    source_cell <- identify_source_cell(ordsDestLocs30m[i,], source_value)
    sourcesForDests30m[i,]<- c(source_cell)
  }
  end<- proc.time()
  total<- end[3]-start[3]
  
  save(sourcesForDests30m,total,file=paste0("probabilisticDownscaling/data/",flow[f],"/simObs/sourcesForDests30mto5m.RData"))
  
  ptFinal<-proc.time()-pt
  time_backLink_FloodArea<-ptFinal[3]
  save(time_backLink_FloodArea, file= paste0("C:/Users/svr5482/probabilisticDownscaling/data/",flow[f],"/time_backLink_FloodArea_30mto5m.RData"))
