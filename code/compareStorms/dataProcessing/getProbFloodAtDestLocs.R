...
for(f in 1:length(flood)){
  ...
  if(dir.exists(paste0("plots/",flood[f]))==F){
  ...
  #get 5m elevs at the destination coords
  elevAtDestCoords5m<- c(as.matrix(extract(dem5m,coords.5mDest)))
  
  ################################################################################
  #Now use the elevation to 
  #determine the probability of being flooded 
  
  #load parameters from the Gaussian processes
  load(paste0("data/",flood[f],"/modelProbFloodbyElev/parsGP.10mto5m.FN1e-5.RData"))
  
  #load training data (elevations and flooding probabilities)
  ...
  #make predictions at 5m elevations of destination cells
  elevToPred<- elevAtDestCoords5m
  nNewCost= length(elevToPred)
  elevToPred.c<- elevToPred/max(elevs)
  
  ################################################################################
  #compute distance matrix with new elevations to predict at
  ...
  save(predProbFlood,
       file=paste0("data/",flood[f],"/modelProbFloodbyElev/predProbFloodatDest_5mElev10mModel.RData"))
  
  ptFinal<-proc.time()-pt
  time_predProbFlood<-ptFinal[3]
  save(time_predProbFlood, file= paste0("data/",flood[f],"/modelProbFloodbyElev/time_predProbFlood10mto5m.RData"))
  
  
  filename<- paste0("plots/",flood[f],"/ProbFloodbyElevFuncAtDestPts10mto5m.jpeg")
  jpeg(file = filename,width = 700,height=700)
  print(plot(elevToPred,predProbFlood,
             main="Probability Flooded = f(Elevation)",
             xlab="Elevation",
             ylab="Interpolated P(Flooded) From 10m Flooded Locs"))
  print(points(elevs,allPctFloodedbyElev10m,col="red",pch=16))
  dev.off()
