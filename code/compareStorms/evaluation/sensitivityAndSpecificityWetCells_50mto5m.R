...
for(f in 1:length(flow)){
  ...
  for(i in 1:length(downscale50m)){
    pNoFlood[i]<- pnorm(0, mean= downscale50m[i], sd = sqrt(varResHWM50m))
  }
  ...
  save(spec_LRW,sens_LRW,nFlood,nNoFlood,flood5mInds,noFlood5mInds,
       ...
       file=paste0("probabilisticDownscaling/data/",flow[f],"/wetCellAccuracy50mto5m.RData"))
  
  pFloodWet<- 1-pNoFlood
  save(pFloodWet,file=paste0("probabilisticDownscaling/data/",flow[f],"/simObs/pFloodWet_50mto5m.RData"))
}
#[1] "Q2559.8429"
