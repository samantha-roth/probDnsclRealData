...
for(f in 1:length(flow)){
  ...
  for(i in 1:length(downscale30m)){
    pNoFlood[i]<- pnorm(0, mean= downscale30m[i], sd = sqrt(varResHWM30m))
  }
  ...
  save(spec_LRW,sens_LRW,nFlood,nNoFlood,flood5mInds,noFlood5mInds,
       ...
       file=paste0("probabilisticDownscaling/data/",flow[f],"/wetCellAccuracy30mto5m.RData"))
  
  pFloodWet<- 1-pNoFlood
  save(pFloodWet,file=paste0("probabilisticDownscaling/data/",flow[f],"/simObs/pFloodWet_30mto5m.RData"))
}
