...
f=1  
  ...
  elev50mat5m<- c(as.matrix(extract(dem50m,coords5m[goodInds5m,])))
  ...
  floodBin50mat5mVals<- c(as.matrix(extract(floodBin50mat5m,coords5m[goodInds5m,])))
  ...
  maxWetElev50m<- max(elev50mat5m[which(floodBin50mat5mVals==1)])
  maxWetElev5m<- max(elev5m[which(floodBin5mVals==1)])
  print(paste0("max wet elevation at 50m: ",maxWetElev50m))
  print(paste0("max wet elevation at 5m: ",maxWetElev5m))
  
  minDryElev50m<- min(elev50mat5m[which(floodBin50mat5mVals==0)])
  minDryElev5m<- min(elev5m[which(floodBin5mVals==0)])
  print(paste0("min dry elevation at 50m: ",minDryElev50m))
  print(paste0("min dry elevation at 5m: ",minDryElev5m))
  
  minNotFlooded<- min(elev50mat5m[which(floodBin50mat5mVals==0)])
  
  maxFlooded<- max(elev50mat5m[which(floodBin50mat5mVals==1)])
  
  ##############################################################################
  
  WetAndDryInds50m<- which(elev50mat5m>minDryElev50m & elev50mat5m<maxWetElev50m)
  #hist(elev50mat5m[WetAndDryInds50m])
  max(elev50mat5m[WetAndDryInds50m])
  min(elev50mat5m[WetAndDryInds50m])
  len<- (max(elev50mat5m[WetAndDryInds50m])-min(elev50mat5m[WetAndDryInds50m]))/13
  
  divs<- seq(minDryElev50m,maxWetElev50m,by=len)
  divs<- c(divs[1:2],divs[5:length(divs)])
  
  elevIntervals50m<- rep(NA, length(elev50mat5m))
  ...
  for(i in 1:(length(divs)-1)){
    ...
    want50m<- which(elev50mat5m>=divs[i] & elev50mat5m<= divs[i+1])
    elevIntervals50m[want50m]<- divs[i]
  ...
  pctFloodedbyElev50m<- rep(NA,length(divs)-1)
  ...
  for(i in 1: 1:(length(divs)-1)){
    ...
    if(length(which(elevIntervals50m==divs[i]))>0){
      pctFloodedbyElev50m[i]<- mean(floodBin50mat5mVals[which(elevIntervals50m==divs[i])])}
    if(length(which(elevIntervals50m==divs[i]))==0) pctFloodedbyElev50m[i]<-0
  }
  
  print(paste0("MAE: ", mean(abs(pctFloodedbyElev50m-pctFloodedbyElev5m))))
  print(paste0("Mean difference: ", mean(pctFloodedbyElev50m-pctFloodedbyElev5m)))
  
  #midpts<- divs[1:(length(divs)-1)]+len/2
  midpts<- divs[1:(length(divs)-1)]+ (divs[2:length(divs)]-divs[1:(length(divs)-1)])/2
  
  filename<- paste0("probabilisticDownscaling/plots/",flow[f],"/ProbFloodbyElev50m.jpeg")
  jpeg(file = filename,width = 700,height=700)
  print(plot(midpts,pctFloodedbyElev50m,
       xlab="Approx. Elev",
  ...
  print(paste0("Difference > .05: ",midpts[which(abs(pctFloodedbyElev50m-pctFloodedbyElev5m)>.05)]))
  print(paste0("Difference > .10: ",midpts[which(abs(pctFloodedbyElev50m-pctFloodedbyElev5m)>.1)]))
  ...
  save(pctFloodedbyElev50m,pctFloodedbyElev5m,len,divs,WetAndDryInds50m,
       minDryElev50m,maxWetElev50m,LB,midpts,
       file=paste0("probabilisticDownscaling/data/",flow[f],"/pctFloodedbyElevAndLB_50mto5m.RData"))
  
  ptFinal<-proc.time()-pt
  time_pctFloodedbyElev<-ptFinal[3]
  save(time_pctFloodedbyElev, file= paste0("C:/Users/svr5482/probabilisticDownscaling/data/",flow[f],"/time_pctFloodedbyElev_50mto5m.RData"))
