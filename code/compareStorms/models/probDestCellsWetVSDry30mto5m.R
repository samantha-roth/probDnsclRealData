...
for(f in 1:length(flow)){
  ...
  save(time_estProbFloodIndsofInterest, 
       file= paste0("probabilisticDownscaling/data/",flow[f],"/simObs/time_estProbFloodIndsofInterest_30mto5m.RData"))
  ...
  ##load the estimated mean from the nearest downscaled flooded cell
  #load("probabilisticDownscaling/data/simObs/shiftdnsclFromSourceToDest30m.RData")
  ...
  #for(i in 1:length(meanFromSourceToDest)){
  #  probleq0GivenNotPtMass[i]<- pnorm(0, mean = meanFromSourceToDest[i], sd = sqrt(varShiftResHWM30m))
  #  totProbleq0[i]<- (1-predProbFlood5mCost[i])+predProbFlood5mCost[i]*probleq0GivenNotPtMass[i]
  #}
  ...
  save(indsProbNotFloodgeq.5,indsProbFloodgeq.5,
       ...
       file=paste0("probabilisticDownscaling/data/",flow[f],"/simObs/estProbFloodIndsofInterest_30mto5m.RData"))
  
  ################################################################################
  pFloodDry<- 1-totProbleq0
  save(pFloodDry,file=paste0("probabilisticDownscaling/data/",flow[f],"/simObs/pFloodDry_30mto5m.RData"))
