...
for(f in 1:length(flow)){
  ...
  load(paste0("probabilisticDownscaling/data/",flow[f],"/simObs/modelProbFloodbyElev/predProbFloodatDest_5mElev50mModel.RData"))
  ...
  CDFatDestPt<- function(sigma, i, num){
    probleqNumGivenNotPtMass<- pnorm(num, mean = meanFromSourceToDest[i], sd = sqrt(varResHWM50m))
  ...
  upper<- max(meanFromSourceToDest)+ 3*sqrt(varResHWM50m)
  ...
  save(CDFmat,file=paste0("probabilisticDownscaling/data/",flow[f],"/simObs/shiftbyelevdnsclCDFmat_50mto5mElev.RData"))
  ...
  save(approxPDFmat,file=paste0("probabilisticDownscaling/data/",flow[f],"/simObs/shiftbyelevdnsclApproxPDFmat_50mto5mElev.RData"))
  ...
  save(meanAtDests,file=paste0("probabilisticDownscaling/data/",flow[f],"/simObs/shiftbyelevdnsclMeanAtDests_50mto5mElev.RData"))
  
  ptFinal<-proc.time()-pt
  time_getCDFandPDFatDests<-ptFinal[3]
  save(time_getCDFandPDFatDests, file=paste0("C:/Users/svr5482/probabilisticDownscaling/data/",flow[f],"/time_getCDFandPDFatDests_50mto5mElev.RData"))
