...
for(f in 1:length(flow)){
  ...
  ##load the downscaled values at the source cells
  #load("probabilisticDownscaling/data/simObs/downscale50mto5mAtSource.RData")
  ...
  ##source mean (given dest not in point mass at zero) to dest
  #meanFromSourceToDest<- rep(NA, length(destInds50m))
  #for(i in 1:length(downscale50m)){
  #  indsMapTo<- which(sourceIndsForDests50m==sourceIndsUnique[i])
  #  meanFromSourceToDest[indsMapTo]<- downscale50m[i]
  #}
  #save(meanFromSourceToDest,file="probabilisticDownscaling/data/simObs/dnsclFromSourceToDest50mto5m.RData")
  ...
  #load("probabilisticDownscaling/data/simObs/shiftdnscl50mAtSource.RData")
  ...
  #meanFromSourceToDest<- rep(NA, length(destInds50m))
  #for(i in 1:length(shiftdnscl50m)){
  #  indsMapTo<- which(sourceIndsForDests50m==sourceIndsUnique[i])
  #  meanFromSourceToDest[indsMapTo]<- shiftdnscl50m[i]
  #}
  
  #save(meanFromSourceToDest,file="probabilisticDownscaling/data/simObs/shiftdnsclFromSourceToDest50m.RData")
  ################################################################################
