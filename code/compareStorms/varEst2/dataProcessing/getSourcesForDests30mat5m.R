...
for(f in 1:length(flow)){
  ...
  load(paste0("probabilisticDownscaling/data/",flow[f],"/simObs/sourcesForDests30mto5m.RData"))
  
  ################################################################################
  #inds of 5m locations we want to get distributions for
  indsOfInterest<- inds.5m[destInds30m]
  ...
  sourceIndsForDests30m<- rep(NA,nrow(sourcesForDests30m))
  for(i in 1:nrow(sourcesForDests30m)){
    ind<- which(ords.5m[,1]==sourcesForDests30m[i,1] & ords.5m[,2]==sourcesForDests30m[i,2])
    sourceIndsForDests30m[i]<- ind
  }
  
  sourceCoordsForDests30m<- coords.5m[sourceIndsForDests30m,]
  sourceCoordsUnique<- coords.5m[unique(sourceIndsForDests30m),]
  sourceIndsUnique<- unique(sourceIndsForDests30m)
  save(sourceCoordsForDests30m,sourceIndsForDests30m,
       sourceCoordsUnique, sourceIndsUnique,
       file=paste0("probabilisticDownscaling/data/",flow[f],"/simObs/sourceIndsCoordsForDests30mto5m.RData"))
  
  ptFinal<-proc.time()-pt
  time_sourcesForDests<-ptFinal[3]
  save(time_sourcesForDests, file= paste0("C:/Users/svr5482/probabilisticDownscaling/data/",flow[f],"/time_sourcesForDests30mto5m.RData"))
