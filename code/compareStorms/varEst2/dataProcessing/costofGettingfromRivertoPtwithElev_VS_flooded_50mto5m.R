...
costbyElev_River50m<- rast("Documents/ArcGIS/Projects/Norristown_HurricaneIda/CostDis_riv50_1.tif")

coords5m_cost<- xyFromCell(costbyElev_River5m,1:ncell(costbyElev_River5m))

#plot(costbyElev_River50m)
...
for(f in 1:length(flow)){
  ...
  floodBin50mat5m<- rast(paste0("probabilisticDownscaling/data/",flow[f],"/bin50mat5m.tif"))
  ...
  #now that we've established the 5m points where 50m and 50m cost values are defined
  ...
  costbyElev_River50mat5mVals<- extract(costbyElev_River50m,coords5m[goodInds5m,])
  costbyElev_River5mVals<- extract(costbyElev_River5m,coords5m[goodInds5m,])
  
  costbyElev_River50mat5mVals<- c(as.matrix(costbyElev_River50mat5mVals))
  ...
  floodBin50mat5mVals<- extract(floodBin50mat5m,coords5m[goodInds5m,])
  floodBin5mVals<- extract(floodBin5m,coords5m[goodInds5m,])
  
  floodBin50mat5mVals<- c(as.matrix(floodBin50mat5mVals))
  floodBin5mVals<- c(as.matrix(floodBin5mVals))
  
  floodBin50mat5mVals<- ifelse(is.na(floodBin50mat5mVals),0,1)
  floodBin5mVals<- ifelse(is.na(floodBin5mVals),0,1)
  
  print(max(costbyElev_River50mat5mVals[which(floodBin50mat5mVals==1)]))
  print(max(costbyElev_River5mVals[which(floodBin5mVals==1)]))
  
  max(costbyElev_River50mat5mVals); max(costbyElev_River5mVals)
  length(which(costbyElev_River50mat5mVals>16000))
  
  len=500
  roundlen<- RoundTo(max(costbyElev_River50mat5mVals),len)
  if(max(costbyElev_River50mat5mVals)>roundlen) topcost<- roundlen+len
  if(max(costbyElev_River50mat5mVals)<=roundlen) topcost<- roundlen
  
  #now if as we get closer to 50000, see if the percentage of flooded cells decreases
  
  divs<- seq(0,topcost,by=len)
  
  costIntervals50m<- rep(NA, length(costbyElev_River50mat5mVals))
  ...
  for(i in 1:(length(divs)-1)){
    ...
    want50m<- which((costbyElev_River50mat5mVals>=divs[i]& costbyElev_River50mat5mVals<= divs[i+1])==T)
    costIntervals50m[want50m]<- divs[i]
    #print(paste0("# 50m cells: ", length(want50m)))
    performByDiv[i,2]<- length(want50m)
  ...
  pctFloodedbyCost50m<- rep(NA,length(divs)-1)
  ...
  for(i in 1: 1:(length(divs)-1)){
    ...
    if(length(which(costIntervals50m==divs[i]))>0){
      pctFloodedbyCost50m[i]<- mean(floodBin50mat5mVals[which(costIntervals50m==divs[i])])}
    if(length(which(costIntervals50m==divs[i]))==0) pctFloodedbyCost50m[i]<-0
  }
  
  #plot(abs(performByDiv[,2]-performByDiv[,3]),abs(pctFloodedbyCost50m-pctFloodedbyCost5m))
  print(mean(abs(pctFloodedbyCost50m-pctFloodedbyCost5m)))
  print(mean(pctFloodedbyCost50m-pctFloodedbyCost5m))
  
  midpts<- divs[1:(length(divs)-1)]+len/2
  
  print(plot(midpts,pctFloodedbyCost50m,
  ...
  print(midpts[which(abs(pctFloodedbyCost50m-pctFloodedbyCost5m)>.05)])
  ...
  #save(pctFloodedbyCost50m,pctFloodedbyCost50m,pctFloodedbyCost5m,LB,
  #     file="probabilisticDownscaling/data/pctFloodedbyCostAndLB.RData")
  
  save(pctFloodedbyCost50m,pctFloodedbyCost5m,LB,
       file=paste0("probabilisticDownscaling/data/",flow[f],"/pctFloodedbyCostAndLB_50mto5m.RData"))
  
  ptFinal<-proc.time()-pt
  time_pctFloodedbyCost<-ptFinal[3]
  save(time_pctFloodedbyCost, file= paste0("C:/Users/svr5482/probabilisticDownscaling/data/",flow[f],"/time_pctFloodedbyCost_50mto5m.RData"))
