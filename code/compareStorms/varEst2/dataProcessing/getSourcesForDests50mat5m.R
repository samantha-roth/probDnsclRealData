...
for(f in 1:length(flow)){
  ...
  load(paste0("probabilisticDownscaling/data/",flow[f],"/simObs/sourcesForDests50mto5m.RData"))
  
  ################################################################################
  #inds of 5m locations we want to get distributions for
  indsOfInterest<- inds.5m[destInds50m]
  ...
  sourceIndsForDests50m<- rep(NA,nrow(sourcesForDests50m))
  for(i in 1:nrow(sourcesForDests50m)){
    ind<- which(ords.5m[,1]==sourcesForDests50m[i,1] & ords.5m[,2]==sourcesForDests50m[i,2])
    sourceIndsForDests50m[i]<- ind
  }
  
  sourceCoordsForDests50m<- coords.5m[sourceIndsForDests50m,]
  sourceCoordsUnique<- coords.5m[unique(sourceIndsForDests50m),]
  sourceIndsUnique<- unique(sourceIndsForDests50m)
  save(sourceCoordsForDests50m,sourceIndsForDests50m,
       sourceCoordsUnique, sourceIndsUnique,
       file=paste0("probabilisticDownscaling/data/",flow[f],"/simObs/sourceIndsCoordsForDests50mto5m.RData"))
  
  ptFinal<-proc.time()-pt
  time_sourcesForDests<-ptFinal[3]
  save(time_sourcesForDests, file= paste0("C:/Users/svr5482/probabilisticDownscaling/data/",flow[f],"/time_sourcesForDests50mto5m.RData"))
