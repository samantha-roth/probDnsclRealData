...
for(f in 1:length(flow)){
  ...
  length(which(good5minds%in%good5minds[c(destIndsinGoodInds50m,floodInds50mat5m)]))
  
  ################################################################################
  #get flooded and not flooded locations within the business region of interest
  floodInds<- good5minds[floodInds50mat5m]
  ...
  load(paste0("probabilisticDownscaling/data/",flow[f],"/simObs/estProbFloodIndsofInterest_50mto5m.RData"))
  ...
  correctFloodDryInds<- destInds50m[indsFloodCorrect]
  correctNoFloodDryInds<- destInds50m[indsNotFloodCorrect]
  incorrectNoFloodDryInds<- destInds50m[setdiff(indsFlood,indsFloodCorrect)]
  incorrectFloodDryInds<- destInds50m[setdiff(indsNotFlood,indsNotFloodCorrect)]
  
  label<- rep(NA, nrow(coords.5m))
  
  #label[floodInds50mat5m]<- "wet"
  #label[-floodInds50mat5m]<- "dry"
  ...
  filename<- paste0("probabilisticDownscaling/plots/",flow[f],"/BusAreaAccSpatSts_50mto5m.jpeg")
