...
for(f in 1:length(flow)){
  ...
  load(paste0("probabilisticDownscaling/data/",flow[f],"/simObs/pFloodDry_50mto5m.RData"))
  
  load(paste0("probabilisticDownscaling/data/",flow[f],"/simObs/pFloodWet_50mto5m.RData"))
  ################################################################################
  
  pFlood<- rep(NA, nrow(coordsinBds.5m))
  
  pFlood[floodInds50mat5m]<- pFloodWet
  pFlood[-floodInds50mat5m]<- pFloodDry
  ...
  filename<- paste0("probabilisticDownscaling/plots/",flow[f],"/BusAreaSpatPFlood_50mto5m.jpeg")
