...
for(f in 1:length(flow)){
  ...
  load(paste0("probabilisticDownscaling/data/",flow[f],"/simObs/pFloodDry_30mto5m.RData"))
  
  load(paste0("probabilisticDownscaling/data/",flow[f],"/simObs/pFloodWet_30mto5m.RData"))
  ################################################################################
  
  pFlood<- rep(NA, nrow(coordsinBds.5m))
  
  pFlood[floodInds30mat5m]<- pFloodWet
  pFlood[-floodInds30mat5m]<- pFloodDry
  ...
  filename<- paste0("probabilisticDownscaling/plots/",flow[f],"/BusAreaSpatPFlood_30mto5m.jpeg")
