...
for(f in 1:length(flow)){
  ...
  CDFatDestPt<- function(sigma, i, num){
    probleqNumGivenNotPtMass<- pnorm(num, mean = meanFromSourceToDest[i], sd = sqrt(predVD30mto5m))
  ...
  upper<- max(meanFromSourceToDest)+ 3*sqrt(predVD30mto5m)
  ...
  for(j in 1:length(valsToTest)){
    for(i in 1:length(meanFromSourceToDest)){
      CDFmat[i,j]<- CDFatDestPt(sigma=sqrt(varResHWM30m),i=i,num=valsToTest[j])
    }
  }
  
  save(CDFmat,file=paste0("probabilisticDownscaling/data/",flow[f],"/simObs/interpVarDnsclErrbyMeanAbsElevDiff/shiftbyelevdnsclCDFmat_30mto5m.RData"))
  ...
  save(approxPDFmat,file=paste0("probabilisticDownscaling/data/",flow[f],"/simObs/interpVarDnsclErrbyMeanAbsElevDiff/shiftbyelevdnsclApproxPDFmat_30mto5m.RData"))
  ...
  mean(meanAtDests[indsProbNotFloodgeq.5]) #0.1098303
  ...
  save(meanAtDests,file=paste0("probabilisticDownscaling/data/",flow[f],"/simObs/interpVarDnsclErrbyMeanAbsElevDiff/shiftbyelevdnsclMeanAtDests_30mto5m.RData"))
  
  ptFinal<-proc.time()-pt
  time_getCDFandPDFatDests<-ptFinal[3]
  save(time_getCDFandPDFatDests, file=paste0("C:/Users/svr5482/probabilisticDownscaling/data/",flow[f],"/simObs/interpVarDnsclErrbyMeanAbsElevDiff/time_getCDFandPDFatDests_30mto5m.RData"))
