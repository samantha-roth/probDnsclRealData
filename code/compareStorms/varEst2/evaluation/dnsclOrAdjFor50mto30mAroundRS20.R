...
for(f in 1:length(flow)){
  ...
  load(paste0("probabilisticDownscaling/data/",flow[f],"/simObs/floodInds50mat30mAroundRS20.RData"))
  ...
  goodLocs<- coordsinBds.30m[floodInds50mat30m,]
  
  y30m<- unique(goodLocs[,2])
  x30m<- unique(goodLocs[,1])
  ...
  elev.30m<- c(as.matrix(extract(dem30m,goodLocs)))
  ...
  downscaled.z.vec<- test- elev.30m
  
  downscale50m<- downscaled.z.vec
  
  WSHtocompare<- c(as.matrix(extract(run30m,goodLocs)))
  
  length(which(downscale50m<0))
  
  downscale50m[which(downscale50m<0)]<- 0
  
  save(downscale50m, file= paste0("probabilisticDownscaling/data/",flow[f],"/simObs/downscale50mto30mAroundRS20.RData"))
  #downscaling improves performance compared to just adjusting here
  
  ptFinal<-proc.time()-pt
  time_downscale50mto30mAroundRS20<-ptFinal[3]
  save(time_downscale50mto30mAroundRS20, file= paste0("probabilisticDownscaling/data/",flow[f],"/simObs/time_downscale50mto30mAroundRS20.RData"))
  ...
  val.50mAdjatFloodInds<- val.50matFloodInds + elev.50matFloodInds - elev.30m
  ...
  VD50to30<- var(discrepancy)
  
  save(VD50to30, file= paste0("probabilisticDownscaling/data/",flow[f],"/simObs/VD50to30.RData"))
