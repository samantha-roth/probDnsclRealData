...
for(f in 1:length(flow)){
  ...
  load(paste0("probabilisticDownscaling/data/",flow[f],"/simObs/compareSLRNoAdj/preds_SLR10mto5m.RData"))
  #load the estimated variance estimated from SLR model for HWMs
  load(paste0("probabilisticDownscaling/data/",flow[f],"/simObs/compareSLRNoAdj/var_SLR10mto5m.RData"))
  ...
  bdsBox10m<- cbind(preds_SLR10m-1.96*sqrt(var_SLR10m),
                    preds_SLR10m+1.96*sqrt(var_SLR10m))
  
  isBtwn5mby10m<- rep(NA,length(vals5minBds))
  
  for(i in 1:length(vals5minBds)){
    isBtwn5mby10m[i]<- vals5minBds[i]>=bdsBox10m[i,1] & vals5minBds[i]<=bdsBox10m[i,2]
  }
  
  print("% time in PI")
  print(mean(isBtwn5mby10m)) #0.8976608
  
  #cor
  cor(preds_SLR10m,vals5minBds) #0.9839154
  #MAE
  print("MAE")
  print(mean(abs(preds_SLR10m-vals5minBds))) #0.1270387
  #MSE
  mean((preds_SLR10m-vals5minBds)^2) #0.03822417
  ...
  for(i in 1:length(vals5minBds)){
    pNoFlood[i]<- pnorm(0, mean= preds_SLR10m[i], sd = sqrt(var_SLR10m))
  }
