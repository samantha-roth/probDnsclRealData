...
load("probDnsclRealData/data/coords.10m.RData")
elev10m<- c(as.matrix(extract(dem10m,coords.10m)))

#get maximum flood heights over all resolutions
maxElev<- max(c(elev5m,elev10m))
################################################################################
...
dem5mDest <- crop(dem5m, extent_to_keep)
coords.5mBox<- xyFromCell(dem5mDest,1:ncell(dem5mDest))
...
df<- data.frame("m"= elev5m,
                "latitude"= lonlat_all[,2],
                "longitude"=lonlat_all[,1])

myPalette <- colorRampPalette(rev(brewer.pal(9, "YlOrRd")))
sc <- scale_colour_gradientn(colours = myPalette(100), limits=c(0, maxElev))

filename<- "probDnsclRealData/plots/dem5m.jpeg"
...
#subset the raster based on the defined extent
dem10mDest <- crop(dem10m, extent_to_keep)
coords.10mBox<- xyFromCell(dem10mDest,1:ncell(dem10mDest))
...
filename<- "probDnsclRealData/plots/dem10m.jpeg"
jpeg(file = filename,width = 590,height=500)
Norristown_plot +
